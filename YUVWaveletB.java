...
public class YUVWaveletB{
 ...
 int deFib(){
     int value = 0;	
     for(value=0; fib[value]<=data[index]; value++)
{		
 ...
 void decode(){
   for (int i = 0; i < height; i++){
    ...
    while (j < width){
     ...
     if (value == 0){
      runLength = deFib();
      for (int l = 0; l < runLength; l++) yuv[i][j + l][0] = 0;
      j += runLength;
