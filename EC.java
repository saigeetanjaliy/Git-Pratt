...
Point multiply(Point P, BigInteger n){
 ...
 void sharedSecret(){  // Figure 10.7 of book
    ...
    Point KB = null;// secret computed by B
 ...
 void encryptionForB(){  // page 305 of book
   ...
   System.out.println(mu.toString(16));// (kG, mu) is the encrypted message
   Point kY2 = multiply(kG,privateKeyB);  // B computes kY as privateKeyB times kG
   BigInteger decodedMessage = mu.multiply(kY2.x.modInverse(p)).mod(p); // kY2.x modinverse times mu mod p
