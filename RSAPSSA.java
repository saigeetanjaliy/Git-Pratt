// RSAPSSA.java CS5125/6025 Cheng 2017
// RSA-PSS encoding 
// Usage:  java RSAPSSA privateKey < message > signature
...
public class RSAPSSA{
   ...
   void encodingStep4(){
     ...
     for (int i = 0; i < border; i++) DB[i] = 0;
     DB[border] = 1;
     for (int i = 0; i < sLen; i++) DB[border + i + 1] = salt[i];
   }
   ...
   void encodingStep8(){
     EM = new byte[emLen];
     for (int i = 0; i < emLen - hLen - 1; i++) EM[i] = DB[i];
     for (int i = 0; i < hLen; i++) EM[emLen - hLen - 1 + i] = H[i];
 ...
 public static void main(String[] args){
   if (args.length < 1){
     System.out.println("Usage: java RSAPSSA privateKey < message > signature");
     return;
   }
   RSAPSSA rsa = new RSAPSSA();
   rsa.readPrivateKey(args[0]);
   rsa.sign();
   rsa.printSignature();
 }
}