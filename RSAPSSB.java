// RSAPSSB.java CS5125/6025 Cheng 2017
...
public class RSAPSSB{
   ...
   void verifyStep4(){  // opposite of encodingStep8, fill DB and H
     // step4.1: allocate array DB: 

     DB = new byte[emLen - hLen - 1]; //length emLen-hLen-1
     ...
     for (i = 0; i < emLen - hLen - 1; i++) DB[i] = EM[i];

     // step4.4: copy H from EM , see encodingStep8

     for (int j = 0; j < hLen; i++,j++)H[j] = EM[emLen - hLen - 1 + j];
   } 
   ...
   void verifyStep9(){  // checking padding2, or part of encodingStep4
     ...
     for (int i = 0; i < border; i++) if (DB[i] != 0){
   ...
   void verifyStep10(){  // opposite of part of encodingStep4, fill salt
     ...
     for (int i = 0; i < sLen; i++)salt[i] = DB[border + i + 1];
   }  
   ...
   void verifyStep13(){ // compare H with H2 or simply print them out
     ...
     if (i < hLen) System.out.println("Inconsistent: verfy step 13");
  ...
  void decrypt(){
     m = s.modPow(e,n);// compute m from s : m is s modPow e n
     EM = m.toByteArray();
     emLen = EM.length;
  }
